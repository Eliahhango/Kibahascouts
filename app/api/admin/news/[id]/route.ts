...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    if (parsedBody.data.slug) {
      const duplicateSlug = await db.collection("news").where("slug", "==", parsedBody.data.slug).limit(1).get()
    ...
    return toApiErrorResponse(error, "Failed to update news")
  }
}
...
export async function DELETE(request: Request, { params }: Params) {
  try {
    ...
    return toApiErrorResponse(error, "Failed to delete news")
  }
}
